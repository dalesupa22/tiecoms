...
final class GroupsTests: XCTestCase {
    ...
    private func boot() throws -> BootstrapDTO {
        ...
        {"contract":"2026-09-25","serverTime":"","me":{"id":"me","name":"Ana Ruiz","kind":"human","primaryOrgId":"oA"},
         ...
         "conversations":[
           ...
           {"id":"der1","workspaceId":"wRel1","kind":"group","name":"Mentoría 1 · seguimiento","parentId":"r1","deriveKind":"same","memberIds":["me","bob"]},
    ...
    func testOrgHomeWithoutHeaderAndOtherOwnSpacesWithHeader() throws {
        let mine = Naming.groupsTree(try boot()).sections[0]
        XCTAssertEqual(mine.companies.count, 1)
        let ws = mine.companies[0].workspaces
        XCTAssertEqual(ws.map(\.id), ["wHome", "wLegacy"], "el espacio casa va primero")
        XCTAssertFalse(ws[0].showHeader, "espacio casa: sus grupos van directo")
        XCTAssertTrue(ws[1].showHeader, "otro espacio solo de mi empresa: con su cabecera")
        XCTAssertEqual(ids(ws[0].convs), ["g1", "g2"], "no leídos primero, luego actividad")
    }

    func testRelationsByCounterpartSingleSpaceHasNoHeaderAndPending() throws {
        ...
        XCTAssertEqual(Set(byId.keys), ["oB", "oD", "pending:nestle"])
        // Ongoing: dos espacios (uno de cada dueño) con cabecera.
        XCTAssertEqual(Set(byId["oB"]!.workspaces.map(\.id)), ["wRel1", "wRel2"])
        XCTAssertTrue(byId["oB"]!.workspaces.allSatisfy(\.showHeader))
        // Delta: un solo espacio, sin cabecera.
        XCTAssertEqual(byId["oD"]!.workspaces.map(\.showHeader), [false])
        // Relación pendiente: nombre escrito, marca y sin organización.
        let p = byId["pending:nestle"]!
        XCTAssertTrue(p.pending)
        XCTAssertNil(p.org)
        XCTAssertEqual(p.name, "Nestlé")
        XCTAssertEqual(ids(p.workspaces[0].convs), ["p1"])
        XCTAssertFalse(byId["oB"]!.pending)
    }
    ...
    func testGuestSectionUnderHostCompany() throws {
        ...
        XCTAssertEqual(ids(g.companies[0].workspaces[0].convs), ["x1"])
        XCTAssertFalse(g.companies[0].workspaces[0].showHeader, "un solo espacio: sin cabecera, como en Relaciones")
    ...
    func testDerivedNestedUnderOrigin() throws {
        let rel = Naming.groupsTree(try boot()).sections.first { $0.kind == .relations }!
        let w = rel.companies.first { $0.id == "oB" }!.workspaces.first { $0.id == "wRel1" }!
        XCTAssertEqual(ids(w.convs), ["r1"], "la derivada no va suelta")
        XCTAssertEqual(w.convs[0].derived.map(\.id), ["der1"])
    }

    func testFiltersAndSearchDropEmptySections() throws {
        let d = try boot()
        let unread = Naming.groupsTree(d, tab: .unread)
        XCTAssertEqual(unread.sections.map(\.id), ["mine:oA"], "silenciada sin mención no cuenta; sin secciones vacías")
    ...
    func testUnreadBadgesPerTab() throws {
        let d = try boot()
        XCTAssertEqual(Naming.groupsUnread(d), 2, "grupos con espacio no silenciados")
        XCTAssertEqual(Naming.dmsUnread(d), 4, "directos y chats, sidechats incluidos")
    }

    func testRelationsForNewGroupAndGuestRole() throws {
        let d = try boot()
        // Delta · Nestlé (pendiente) · Ongoing, por nombre; ni mi organización ni donde soy tercero.
        XCTAssertEqual(Naming.relations(d).map(\.id), ["oD", "pending:nestle", "oB"])
        XCTAssertEqual(Naming.relations(d).last?.workspaces.count, 2)
