...
export async function notifyReport(reportId: string) {
  ...
  const text = `Hay un reporte de seguridad pendiente en Chaggu. Referencia: ${reportId}. Revisar la cola de moderación del servidor según docs/MODERATION.md.`;
  await sendMail({ to: [{ email: process.env.MODERATION_EMAIL ?? 'admin@chaggu.com' }], subject: `Chaggu: reporte ${reportId}`, text,
